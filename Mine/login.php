...
if ($conn->connect_errno != 0) {
    ...
    echo '<br> <a href="index.php">Wróć do strony logowania</a>';
} else {
    $is_okay = true;
    if (isset($_POST['login'])) {
        $login = $_POST['login'];
        $_SESSION['cheaking_login'] = $login;
        if (!filter_var($login, FILTER_VALIDATE_EMAIL)) {
            $_SESSION['login_e'] = 'Błędna składnia e-mail';
            $is_okay = false;
        }
        if($login == ''){
            $_SESSION['login_e'] = 'Nie podano loginu';
            $is_okay = false;
        }
    }else{
        $is_okay = false;
    ...
    if ($result->num_rows == 1) {
        ...
        if (password_verify($haslo,$wiersz['Haslo'])){
            ...
            $_SESSION['haslo_e'] = 'Błędne hasło';
            header('Location: index.php');
            exit();
        }
    } else {
        $_SESSION['alert'] = 'Nie odnaleziono urzytkownika w bazie danych';
        header('Location: index.php');
        exit();
    }
}